...
>截至目前，我们了解到，ACGFAKA 在大部分插件采取收费模式的情况下，仍由其官方使用 ChatGPT 扫描使用者站点内容，并同时收集服务器相关特征信息。我们认为，该行为已经超出必要限度。
...
需要说明的是，我们对这个项目被外挂，黑灰产业滥用表示深感遗憾，我们同情并理解你们的遭遇。但是在源码中过度收集信息与滥用权限这与我们的观念不符。
...
- 后续会对此项目进行完整离线化（截止README提交时间尚未完整本地化），去除后台广告拉取逻辑，并允许普通用户自行编写插件、上传插件并在本地直接使用以及令人期待的基于github的插件商城(完全免费开源代替官方的圈钱垃圾插件)。
